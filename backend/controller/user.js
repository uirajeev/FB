...
export const register = async (req, res) => {
    try {
        ...
        const newUserName = await generateUsername(first_name.toLowerCase() + last_name.toLowerCase());
        ...
        user.save().then(doc => {
            ...
            res.json(crateSession(doc, '7d', 'Congratulations on your successful registration! Please check your email for the activation link.'));
        });
    } catch (error) {
...
export const login = async (req, res) => {
    try {
        ...
        if (!user) {
            return res.status(400).json({ message: "We couldn't find this email address in our records. Please check the email address you provided." });
        }
        const checkPassword = await bcrypt.compare(password, user.password);
        if (!checkPassword) {
            return res.status(400).json({ message: "Sorry, the password you entered is invalid. Please try again." });
        }
